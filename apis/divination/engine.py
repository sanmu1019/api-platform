...
#!/usr/bin/env python3
"""梅花易数排盘"""
import datetime, time, json, os, random, urllib.request
...
def _load_iching_data():
    global _DATA_LOADED, _RAW
    if _DATA_LOADED:
        return
    _ensure_iching_data()
    ...
    for item in _RAW:
        ...
        _GUA_INDEX[(upper_num, lower_num)] = item
    _DATA_LOADED = True
...
class PlumBlossom:
    ...
    def number_divination(self, number: int, month: int = None, hour: int = None) -> Dict:
        """
        ② 数字起卦：传入一个三位整数
        - 上卦：百位数字 除以8取余
        ...
        `hour` 不传则用服务器当前小时。动爻（进而变卦、体用、吉凶）取决于时辰，
        不把 hour 暴露出去的话，同一个数字在不同时辰会得到不同的卦、而且调用方
        无法复现 —— 这是移植时漏掉的参数。注意干支与应期仍取当前年月日，
        只有 hour 固定时它们才会随之固定。
        """
        ...
        if hour is None:
            hour = now.hour
        _, shichen_num = _get_shichen(hour)
        ...
        return self._build_result(
            upper, lower, mv,
            "数字", str(number),
            month, now.year, now.month, now.day, hour
        )
    ...
    def format_output(self, result: Dict, question: str = "") -> str:
        ...
        # ── 第1行：起卦时间 ──
        # 用 result 里的年月日时，而不是 datetime.datetime.now()：否则同一个
        # number+hour 两次调用会因为请求时刻的秒数不同而产出不同的 text，
        # "可复现"就只对结构化字段成立、对 text 不成立。不传时间时 result 里
        # 装的就是请求时刻，两者等价。
        lines.append(
            f"{result['year']}-{result['month2']}-{result['day']} {result['hour']:02d}:00"
        )
...
def _hexagram_from_bits(bits):
    """Return the stored hexagram record for six lines ordered bottom to top."""
    _load_iching_data()
    lower = _bits_to_trigram_num(bits[:3])
    upper = _bits_to_trigram_num(bits[3:6])
    return _GUA_INDEX.get((upper, lower)), upper, lower


def _fortune_hint(*texts):
    """Provide a conservative text-only tendency hint from the source wording."""
    text = "".join(str(item or "") for item in texts)
    if "大凶" in text or "终凶" in text:
        return "大凶"
    if "元吉" in text or "大吉" in text:
        return "大吉"
    score = 0
    score += sum(text.count(word) for word in ("吉", "亨", "利", "无咎"))
    score -= sum(text.count(word) for word in ("凶", "悔", "吝", "厉"))
    if score >= 3:
        return "吉"
    if score <= -3:
        return "凶"
